...
int api_Initialize(u16 address, u8 dataEnablePolarity, u16 sfrClock, u8 force)
{
	 ...
	 */
	dtd_Fill(&dtd, 16, 60000); /* VGA */
	videoParams_Reset(&params);
	videoParams_SetHdmi(&params, TRUE); /* DVI */
	videoParams_SetDtd(&params, &dtd);
	videoParams_SetEncodingIn(&params, RGB);
	videoParams_SetEncodingOut(&params, RGB);
	videoParams_SetColorResolution(&params, 8);
	videoParams_SetDtd(&params, &dtd);
	videoParams_SetPixelRepetitionFactor(&params, 0);
	videoParams_SetPixelPackingDefaultPhase(&params, 0);
	videoParams_SetColorimetry(&params, 1);
	videoParams_SetHdmiVideoFormat(&params, 0);
	videoParams_Set3dStructure(&params, 0);
	videoParams_Set3dExtData(&params, 0);
	
	audioParams_Reset(&audio);
	audioParams_SetInterfaceType(&audio, I2S);
	audioParams_SetCodingType(&audio, PCM);
	audioParams_SetChannelAllocation(&audio, 0x0B);
	audioParams_SetPacketType(&audio, AUDIO_SAMPLE);
	audioParams_SetSampleSize(&audio, 16);
	audioParams_SetSamplingFrequency(&audio, 96000);
	...
	if (audio_Configure(api_mBaseAddress, &audio, videoParams_GetPixelClock(
                &params), videoParams_GetRatioClock(&params)) != TRUE)
	{       
        	return FALSE;
	}
	...
	if (packets_Configure(api_mBaseAddress, &params, NULL) != TRUE)
	{
		return FALSE;
	}
	/*
	 ...
	 */
	if (control_Configure(api_mBaseAddress, videoParams_GetPixelClock(&params),
			videoParams_GetPixelRepetitionFactor(&params),
			videoParams_GetColorResolution(&params),
			videoParams_IsColorSpaceConversion(&params), 1, 0, 0) != TRUE)
	{
		return FALSE;
	}
	/* send AVMUTE SET (optional) */
	if (phy_Configure (api_mBaseAddress, videoParams_GetPixelClock(&params),
			videoParams_GetColorResolution(&params),
			videoParams_GetPixelRepetitionFactor(&params)) != TRUE)
	{
		return FALSE;
	}
	...
	/* irq handle will be register in later */
#if 0
		...
		productParams_t * product, hdcpParams_t * hdcp)
{
	...
	if (hdcpOn)
	{
		printf("phy_HotPlugDetected %d\n", phy_HotPlugDetected(api_mBaseAddress));
...
void api_EventHandler(int irq, void *param)
{
	u8 i = 0;
	u8 state = 0;
	u8 handled = FALSE;
